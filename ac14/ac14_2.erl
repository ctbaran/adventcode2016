...
generate_hash(Salt, Index) ->
	Start = Salt ++ integer_to_list(Index),
	Bin = erlang:md5(Start),
	stretch_hash(Bin, 0).

stretch_hash(Hash, 2016) -> binary_to_hex(Hash, []);
stretch_hash(Hash, N) -> stretch_hash(erlang:md5( binary_to_hex(Hash, []) ), N+1).
