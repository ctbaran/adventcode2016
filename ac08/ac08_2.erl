...
print_dict(?XBOUND, ?YBOUND, Dict) -> ok;
print_dict(?XBOUND, Y, Dict) ->
	case dict:fetch({?XBOUND, Y}, Dict) of
		on -> io:format("#\n");
		off -> io:format(".\n")
	end,
	print_dict(0, Y+1, Dict);
print_dict(X, Y, Dict) ->
	case dict:fetch({X, Y}, Dict) of
		on -> io:format("#");
		off -> io:format(".")
	end,
	print_dict(X+1, Y, Dict).
