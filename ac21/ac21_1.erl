...
read_instruction(["rotate",Direction,X,_]) ->
    case Direction of
        "left" -> {rotate_direction,left,list_to_integer(X)};
        "right" -> {rotate_direction,right,list_to_integer(X)}
    end;
...
read_instruction(["move","position",X,"to","position",Y]) ->
    {move_position, list_to_integer(X), list_to_integer(Y)}.
...
do_instruction(Password, {rotate_letter, X}) -> rotate_letter(Password, X);
